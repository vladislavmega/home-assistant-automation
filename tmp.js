...
async function isAvailable() {
    ...
    var pingBoiler = new Request('https://prml.kyiv.ua/state');
    ...
    const test = Promise.allSettled([routerPromise, boilerPromise, ISPPromise]).then((values) => {
        ...
        return [(values[0] && values[0].status === 'fulfilled'), (values[1] && values[1].status === 'fulfilled'), ISP, since];
    });
...
async function run() {
    ...
    return isAvailable().then((values) => {
        const [isRouterAvailable, isBoilderAvailable, ispName, since] = values;
        let message = '';
